...
class Teleop(Node):
    def __init__(self):
        ...
        self.declare_parameter("turn", 10)
        ...
        # Only create joy subscriber if in joy mode
        if self.use_joy:
            ...
            # Initialize terminal settings only for keyboard mode
            self.settings = termios.tcgetattr(sys.stdin)
            self.print_keyboard_message()
            self.poll_keys()

    def print_keyboard_message(self):
        self.msg = """
...
CTRL-C to quit
    ...
    def joy_callback(self, data):
        ...
        twist.linear.x = float(data.axes[1]) * float(self.speed)
        twist.linear.y = float(data.buttons[4]) * float(data.axes[0]) * float(self.speed)
        twist.linear.z = 0.0
        twist.angular.x = 0.0
        twist.angular.y = 0.0
        twist.angular.z = float(not data.buttons[4]) * float(data.axes[0]) * float(self.turn)
        ...
        body_pose_lite.roll = float(not data.buttons[5]) * float(-data.axes[3]) * 0.349066
        body_pose_lite.pitch = float(data.axes[4]) * 0.174533
        body_pose_lite.yaw = float(data.buttons[5]) * float(data.axes[3]) * 0.436332
        if data.axes[5] < 0:
            body_pose_lite.z = float(data.axes[5]) * 0.5
        else:
        ...
        body_pose.position.z = float(body_pose_lite.z)

        quaternion = quaternion_from_euler(body_pose_lite.roll, body_pose_lite.pitch, body_pose_lite.yaw)
        body_pose.orientation.x = float(quaternion[0])
        body_pose.orientation.y = float(quaternion[1])
        body_pose.orientation.z = float(quaternion[2])
        body_pose.orientation.w = float(quaternion[3])
